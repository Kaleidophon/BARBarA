...
\chapter{Evaluation der Wortkontextvektoren} % Main chapter title
...
online unter \url{http://bookworm.benschmidt.org/posts/2015-10-25-Word-Embeddings.html} (zuletzt abgerufen am 21.04.15)}}
\end{itquote}


\section{Evaluationsarten für Wortkontextvektoren}

An dieser Stelle sollen die verschiedenen Ansätze zur Evaluation der Wortkontextvektoren, die im vorherigen Kapitel
...
Diese Frage meist dahingehend beantwortet, dass Vektoren dann als überlegen anzusehen sind, wenn sie
...
angewendet und kritisch reflektiert werden sollen.\\
  ...
  Auge leicht zu interpretierende Ergebnisse liefern. Deshalb sind sie für einen ersten Eindruck
  auch durchaus geeignet, sollten wenn möglich aber nicht als alleiniges Kriterium für eine Bewertung
  ...
  Unter der Annahme, dass in Vektorräumen die Wortkontextvektoren ähnlicher Wörter nahe beieinanderliegen, sollte diese Liste
  ...
  \end{changemargin}
    \caption[Listen der $k=5$ nächsten Nachbarn von Wörtern in verschiedenen Datensets]{Listen der $k=5$ nächsten Nachbarn von Wörtern in verschiedenen Datensets. Inspiriert
  ...
  die Ergebnisse verschiedener Datensets zu vergleichen, da sich die Qualität der \emph{k} Nachbarn
  ...
  Darum ist festzuhalten, dass sich qualitative Methoden in diesem Fall eher für den ersten Eindruck
  eignen, weiterhin aber Prozeduren mit quantifizierbaren Ergebnissen verwendet werden sollten, die beispielsweise
  ...
  \begin{enumerate}
    \item \textbf{Benchmark-Tests}\\
      Bei dieser pragmatischen Art der Bewertung wird das Datenset als Grundlage für
      einfache Aufgaben wie Sentiment-Klassifikation oder Part-of-Speech-Tagging verwendet.
      Die Qualität der Daten wird dann anhand der Ergebnisse des Systems gemessen.\\
      Diese extrinsische Evaluationsmethode macht also nur dann Sinn, wenn man mehr als ein Datenset
    ...
    \item \textbf{Wortähnlichkeit}\\
      ...
      werden mit den zur Verfügung stehenden Vektoren Ähnlichkeitswerte für die gleichen Paare berechnet,
      ...
      \end{equation}
      Im Folgenden kann mit \emph{Spearman's $\rho$}
      ...
      festgestellt werden, ob die beiden Werte für die Wortpaare korrelieren, also ob
    ...
    \item \textbf{Analogien}\\
      ...
      \end{equation}
      Dabei ist $\epsilon = 0,001$, um die Divison durch Null zu verhindern\footnote{Um nur Werte im Bereich $[0,1]$ zu erhalten,
      wird die Formel für die Kosinusähnlichkeit zudem folgendermaßen angepasst:
      \[
        cos'(\vec{a}, \vec{b}) = \frac{cos(\vec{a}, \vec{b}) + 1}{2}
      \]}.\\
      Die Performanz des Systems kann dann als Anteil der richtig vervollständigten Analogien (bei denen $\tilde{b} = b'$) gemessen werden.
  \end{enumerate}
  ...
  die genannten Evaluationsdatensets nur dann aussagekräftige Ergebnisse, wenn die menschlichen
  Annotatoren bei der Wortähnlichkeit der Paare zuverlässig und sinnvoll bewertet haben (zu messen z.B. mit $Cohen's\ \kappa$).
  Auch auf die Zusammenstellung der Analogien ist zu achten (Welche Entitäten sind enthalten? Wie oft kommen diese
  im Datenset vor? Welche semantischen Relationen wurden ausgewählt? Wurden diese maschinell oder per Hand erzeugt?).\\
  ...
  \subsection{Evaluationsdaten}\label{sec:evaldata}
    ...
    verwendet. Dieses wurde unter dem Namen \textsc{Schm280} ins Deutsche portiert, wobei die Paare nicht nur einfach übersetzt, sondern die
    ...
    jedoch nicht festgestellt werden, wieviele Annotatoren die Wortpaare neu bewertet haben. Während des Übersetzungsschritts
    ...
    \textsc{SemRel} wurde aus Synonomie-, Antonomie- und Hyperonomie-Beziehungen von (\cite{koper2015multilingual}) für das Deutsche und Englische
    konstruiert. Dabei werden Substantive, Verben und Adjektive berücksichtigt. In der deutschen Variante sind 2.462
    (recht schwierig zu vervollständigende) Analogien enthalten, die aus teilweise sehr seltenen Wörtern kreiert wurden
  ...
  Übersicht findet sich im Appendix \ref{AppendixB}.\\
...
%Mark XIV   & -0,8106$_{(1)}$          & -0,4953$_{(102)}$           & -0,7362$_{(132)}$           & \textbf{-0,7205$_{(48)}$} & 14,51           & 2,56 \\
  ...
  trainiert wurden, als auch die Ergebnisse mit steigender Downsamplingrate schlechter wurden. Dies war sowohl beim Training mit dem CBOW- als
  ...
  Schritte nicht durchgeführt werden konnten, was anhand der im Subskript in Klammern stehenden Zahlen abzulesen ist.
  Zwar erschwert dies die Vergleichbarkeit; es erscheint aber zumindest logisch, dass bei gleichem Evaluationswert
  einem Datenset mit weniger unauffindbaren Wortkontextvektoren eine höhere Qualität beizumessen ist. Je nach
  späterem Anwendungsgebiet könnte diese Unterscheidung aber nicht vonnöten sein, sofern beispielsweise für flektierte
  Formen gar keine Vektoren benötigt werden.\\
  Bei den Analogie-Datensets sind ähnliche Tendenzen sichtbar: Beim \textsc{Google}-Datenset sind die Diskrepanzen jedoch
  deutlich stärker und die Vektoren des unlemmatisierten Korpus schneiden deutlich besser ab, was vor allem dadurch
  ...
  Bei \textsc{Wortpaare222} prägten sich die Korrelationswerte nicht unter $-0,54$ aus, was dafür spricht, das die Ähnlichkeit
  der Wortpaare sowohl für das System als möglicherweise auch für die menschlichen Annotatoren schwer einzuschätzen war
  ...
  Dies führt dazu, dass keine klare Tendenz in den Resultaten zu erkennen ist und allgemein verbesserungswürdige Werte zustande kommen.\\

  Für die nachfolgenden Schritte wurden die am besten abschneidenen Wortkontextvektorsets bei \textsc{Google} und den anderen
